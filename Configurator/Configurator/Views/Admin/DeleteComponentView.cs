...
namespace Configurator.Views.Admin
{
    class DeleteComponentView : IView
    {
        ...
        private void DeleteComponent<T>(IComponentRepository<T> componentRepository) where T : PCComponent
        {
            {
                int id = ConsoleInput.ReadInteger("Введите id комплектующего для удаления: ");
